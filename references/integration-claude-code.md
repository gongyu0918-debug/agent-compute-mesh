...
- project: `./.claude/skills/agent-travel-net/SKILL.md`
- personal: `~/.claude/skills/agent-travel-net/SKILL.md`
    ...
    "Stop": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "\"$CLAUDE_PROJECT_DIR/.claude/hooks/agent-travel-net.ps1\"",
            "async": true,
            "timeout": 120,
            "shell": "powershell"
          }
...
The hook should decide whether the last task qualifies, then update `./.agents/agent-travel-net/suggestions.md`.
Run `agent-travel-net` with `solo-travel` first, medium search by default, all available search tools, and active-conversation-only output.

## Memory Placement

Claude auto memory is for stable project knowledge and user preferences. `agent-travel-net` output is short-lived external advice, so keep it in the advisory file instead of `MEMORY.md`.
